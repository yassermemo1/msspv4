...
import { insertCertificateOfComplianceSchema, type CertificateOfCompliance, type InsertCertificateOfCompliance } from "@shared/schema.ts";
...
export function COCForm({ coc, onSubmit, onCancel, isLoading = false, clientId, contractId, safId, initialData }: COCFormProps) {
  const form = useForm<COCFormData>({
    defaultValues: {
      ...
      issueDate: coc?.issueDate ? dateToString(coc.issueDate) : undefined,
      expiryDate: coc?.expiryDate ? dateToString(coc.expiryDate) : undefined,
      contractId: initialData?.contractId || coc?.contractId || contractId || null,
      status: initialData?.status || coc?.status || "issued",
      auditDate: coc?.auditDate ? dateToString(coc.auditDate) : undefined,
      nextAuditDate: coc?.nextAuditDate ? dateToString(coc.nextAuditDate) : undefined,
  ...
  const handleSubmit = (data: COCFormData) => {
    ...
    const formData: InsertCertificateOfCompliance = {
      ...
      nextAuditDate: data.nextAuditDate ? new Date(data.nextAuditDate) : undefined,
    };
    onSubmit(formData);
  };

...
exports.testDataGenerator = exports.SecureTestDataGenerator = void 0;
...
class SecureTestDataGenerator {
    ...
    generateSecurePassword(length = 16) {
        ...
        const randomArray = (0, crypto_1.randomBytes)(length);
    ...
    generateTestClient() {
        return {
            ...
            size: faker_1.faker.helpers.arrayElement(['1-50', '51-200', '201-500', '500+']),
            status: faker_1.faker.helpers.arrayElement(['active', 'prospect', 'inactive']),
            acquisitionChannel: faker_1.faker.helpers.arrayElement(['direct', 'referral', 'marketing', 'partner']),
    ...
    generateTestContract() {
        const startDate = faker_1.faker.date.future();
        const endDate = faker_1.faker.date.future({ years: 1, refDate: startDate });
        return {
            name: `${faker_1.faker.company.buzzNoun()} ${faker_1.faker.helpers.arrayElement(['Agreement', 'Contract', 'Service Agreement'])}`,
            startDate: startDate.toISOString().split('T')[0],
            endDate: endDate.toISOString().split('T')[0],
            value: faker_1.faker.number.float({ min: 10000, max: 500000, fractionDigits: 2 }),
            status: faker_1.faker.helpers.arrayElement(['active', 'draft', 'expired', 'terminated']),
            autoRenewal: faker_1.faker.datatype.boolean(),
            terms: faker_1.faker.lorem.sentences(2)
        };
    }
    generateTestService() {
        return {
            name: `${faker_1.faker.hacker.adjective()} ${faker_1.faker.helpers.arrayElement(['Security', 'Monitoring', 'Assessment', 'Management'])}`,
            category: faker_1.faker.helpers.arrayElement(['Security Operations', 'Compliance', 'Network Security', 'Cloud Security']),
            description: faker_1.faker.lorem.paragraph(),
            deliveryModel: faker_1.faker.helpers.arrayElement(['on-site', 'remote', 'hybrid']),
            basePrice: faker_1.faker.number.float({ min: 1000, max: 50000, fractionDigits: 2 }),
            pricingModel: faker_1.faker.helpers.arrayElement(['per month', 'per assessment', 'per user', 'fixed price']),
    ...
    generateTestUsers(count = 10) {
        ...
        for (let i = 0; i < count - 4; i++) {
            ...
            users.push({
                ...
                firstName: faker_1.faker.person.firstName(),
                lastName: faker_1.faker.person.lastName(),
                role: faker_1.faker.helpers.arrayElement(['engineer', 'user']),
...
exports.SecureTestDataGenerator = SecureTestDataGenerator;
exports.testDataGenerator = SecureTestDataGenerator.getInstance();

...
class EmailService {
    ...
    generateContractReminderTemplate(contract, client, daysUntilExpiry) {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    generateFinancialAlertTemplate(transaction, client) {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    generateGeneralNotificationTemplate(title, message, client) {
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
  <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
    ...
    async sendContractReminderNotification(contractId, userId) {
        try {
            ...
            const contract = await storage_1.storage.getContract(contractId);
            if (!contract) {
                console.error('Contract not found:', contractId);
                return false;
            }
            const client = await storage_1.storage.getClient(contract.clientId);
            if (!client) {
    ...
    async sendFinancialAlertNotification(transactionId, userId) {
        try {
            const user = await storage_1.storage.getUser(userId);
            if (!user?.email) {
                console.error('User email not found:', userId);
            ...
            const transactions = await storage_1.storage.getAllFinancialTransactions();
            ...
            if (transaction.clientId) {
                client = await storage_1.storage.getClient(transaction.clientId);
            }
            if (!client) {
    ...
    async sendGeneralNotification(userId, title, message, clientId) {
        try {
            const user = await storage_1.storage.getUser(userId);
            if (!user?.email) {
                console.error('User email not found:', userId);
            ...
            let client;
            if (clientId) {
                client = await storage_1.storage.getClient(clientId);
            }
    ...
    async checkExpiringContracts() {
        try {
            const contracts = await storage_1.storage.getAllContracts();
            ...
            for (const contract of contracts) {
                ...
                if (reminderDays.includes(daysUntilExpiry)) {
                    ...
                    try {
                        const teamAssignments = await storage_1.storage.getClientTeamAssignments(contract.clientId);

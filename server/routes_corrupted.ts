...
// Create service scope
  ...
  app.get("/api/audit-logs", requireAdmin, async (req, res) => {
    try {
      ...
      // Apply filters (call .where at most once)
      let filteredQuery = baseQuery;
      if (actionFilter !== 'all') {
        filteredQuery = filteredQuery.where(eq(auditLogs.action, actionFilter as string));
      }
      if (searchTerm) {
  ...
  app.get("/api/data-access-logs", requireAdmin, async (req, res) => {
    try {
      ...
      const logs = await filteredQuery
        .where(gte(dataAccessLogs.timestamp, startDate))
  ...
  app.get("/api/change-history", requireAdmin, async (req, res) => {
    try {
      ...
      const history = await filteredQuery
        .where(gte(changeHistory.timestamp, startDate))
  ...
  app.get("/api/audit/logs", requireAuth, async (req, res) => {
    try {
      ...
      if (entityType && entityId) {
        ...
        filteredQuery = filteredQuery.where(eq(auditLogs.entityType, entityType as string));
      }
  ...
  app.get("/api/audit/change-history", requireAuth, async (req, res) => {
    try {
      ...
      if (entityType && entityId) {
        ...
        filteredQuery = filteredQuery.where(eq(changeHistory.entityType, entityType as string));
      }

      const changes = await filteredQuery
        .orderBy(desc(changeHistory.timestamp))
        .limit(100);
    ...
    try {
      ...
      if (entityType && entityId) {
        filteredQuery = baseQuery.where(
          and(
            eq(securityEvents.entityType, entityType as string),
            eq(securityEvents.entityId, parseInt(entityId as string))
          )
        );
      } else if (entityType) {
        filteredQuery = baseQuery.where(eq(securityEvents.entityType, entityType as string));
      }
  ...
  app.get("/api/audit/data-access", requireAuth, async (req, res) => {
    try {
      ...
      if (entityType && entityId) {
        ...
        filteredQuery = baseQuery.where(eq(dataAccessLogs.entityType, entityType as string));
      }
  ...
  app.post("/api/audit/rollback/:id", requireAdmin, async (req, res) => {
    try {
      ...
      await performRollback(change.rollbackData);
  ...
  app.post("/api/data-sources/:id/sync", requireAuth, async (req, res) => {
    try {
      ...
      // Fetch mappings for this data source
      const mappings = await storage.getDataSourceMappings(id);
      ...
      try {
        ...
        if (syncResponse.ok) {
          const rawRecord = await syncResponse.json();
          
          // Apply field mappings - ensure data is in an array for the mapper
          const mappedRecord = applyMappings([rawRecord], mappings)[0];
          ...
          const integratedData = await storage.createIntegratedData({
            ...
            recordIdentifier: mappedRecord.id || rawRecord.id || `sync_${Date.now()}`
          });

          res.json({
            success: true,
            message: "Data synchronized and mapped successfully",
  ...
  // External Widget Routes - DEPRECATED (migrated to plugin system)
  app.all("/api/external-widgets*", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External widgets have been migrated to plugins. Use /api/integration-engine-widgets instead.",
      migration: {
        newEndpoint: "/api/integration-engine-widgets",
        documentation: "See plugin system documentation for migration guide"
      }
    });
  });
  app.use("/api/integration-engine-widgets", integrationEngineWidgetRoutes);
  
  // Plugin Routes (simplified architecture without external systems)
  ...
  // ========================================
  // EXTERNAL SYSTEMS ENDPOINTS - DEPRECATED
  // ========================================
  // These endpoints have been replaced by the plugin system
  // Keeping minimal stubs for backward compatibility during migration
  
  // Redirect external systems requests to plugins
  app.get("/api/external-systems", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External systems have been migrated to plugins. Use /api/plugins instead.",
      migration: {
        newEndpoint: "/api/plugins",
        documentation: "See plugin system documentation for migration guide"
      }
    });
  });
  ...
  // Enhanced query testing endpoint - DEPRECATED
  app.post("/api/external-systems/test-query", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External systems have been migrated to plugins. Use /api/integration-engine-widgets/:pluginName/:instanceId/execute-query instead.",
      migration: {
        newEndpoint: "/api/integration-engine-widgets/:pluginName/:instanceId/execute-query",
        documentation: "See plugin system documentation for migration guide"
      }
    });
  });

  // Get external system by ID - DEPRECATED
  ...
  // Create external system - DEPRECATED
  app.post("/api/external-systems", requireManagerOrAbove, async (req, res) => {
    res.status(410).json({ 
      message: "External systems have been migrated to plugins. Use plugin configuration files instead.",
      migration: {
        newEndpoint: "/api/plugins",
        documentation: "See plugin system documentation for migration guide"
      }
  ...
  // Update external system - DEPRECATED
  ...
  // Delete external system - DEPRECATED
  ...
  // External mappings endpoints - DEPRECATED
  app.all("/api/external-mappings*", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External mappings have been migrated to plugins. Client-plugin associations are now managed through plugin configuration files.",
      migration: {
        newEndpoint: "/api/plugins",
        documentation: "See plugin system documentation for migration guide"
      }
  ...
  app.all("/api/clients/:id/external-mappings*", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External mappings have been migrated to plugins. Client-plugin associations are now managed through plugin configuration files.",
      migration: {
        newEndpoint: "/api/plugins",
  ...
  // Get aggregated data for client - DEPRECATED
  app.get("/api/clients/:id/aggregated-data", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "Aggregated data endpoints have been migrated to plugins. Use plugin-specific data endpoints instead.",
      migration: {
        newEndpoint: "/api/integration-engine-widgets/:pluginName/:instanceId/execute-query",
        documentation: "See plugin system documentation for migration guide"
      }
  ...
  // Test external system connection - DEPRECATED
  app.post("/api/external-systems/:id/test-connection", requireAuth, async (req, res) => {
    res.status(410).json({ 
      message: "External systems have been migrated to plugins. Use /api/integration-engine-widgets/:pluginName/:instanceId/test-connection instead.",
      migration: {
        newEndpoint: "/api/integration-engine-widgets/:pluginName/:instanceId/test-connection",
        documentation: "See plugin system documentation for migration guide"
      }
    });
  });

  // Execute dynamic query on external system - DEPRECATED
  ...
  // Get global widgets specifically for client detail pages
  app.get("/api/global-widgets", requireAuth, async (req, res) => {
    try {
      const widgets = await db
        .select()
        .from(externalWidgetTemplates)
        ...
        .where(
          and(
            eq(externalWidgetTemplates.isGlobal, true),
            eq(externalWidgetTemplates.isActive, true)
          )
        )
        .orderBy(externalWidgetTemplates.position, desc(externalWidgetTemplates.createdAt));
      
      const formattedWidgets = widgets.map(row => ({
        id: row.externalWidgetTemplates.id,
        systemId: row.externalWidgetTemplates.systemId,
        systemName: row.externalSystems.name,
        pluginName: row.externalSystems.systemName,
        name: row.externalWidgetTemplates.name,
        description: row.externalWidgetTemplates.description,
        widgetType: row.externalWidgetTemplates.widgetType,
        chartType: row.externalWidgetTemplates.chartType,
        query: row.externalWidgetTemplates.query,
        method: row.externalWidgetTemplates.method,
        parameters: row.externalWidgetTemplates.parameters,
        displayConfig: row.externalWidgetTemplates.displayConfig,
        refreshInterval: row.externalWidgetTemplates.refreshInterval,
        isActive: row.externalWidgetTemplates.isActive,
        isGlobal: row.externalWidgetTemplates.isGlobal,
        position: row.externalWidgetTemplates.position,
        createdBy: row.externalWidgetTemplates.createdBy,
      ...
      res.status(500).json({ message: "Failed to fetch global widgets" });
    }
  });

  // Toggle global status of a widget
  app.patch("/api/external-widgets/:id/toggle-global", requireAuth, async (req, res) => {
    try {
      const id = parseInt(req.params.id);
      const userId = req.user.id;
      const userRole = req.user.role;
      ...
      const [updatedWidget] = await db
        .update(externalWidgetTemplates)
        .set({
          isGlobal: !existingWidget[0].isGlobal,
          updatedAt: new Date()
        })
        .where(eq(externalWidgetTemplates.id, id))
        .returning();
      ...
      res.status(500).json({ message: "Failed to toggle global status" });
    }
  });

  // Get all widgets (global and non-global) for management
  app.get("/api/widgets/manage", requireAuth, async (req, res) => {
    try {
      const userId = req.user.id;
      const userRole = req.user.role;
      ...
      console.error('Failed to fetch widgets for management:', error);
      res.status(500).json({ message: "Failed to fetch widgets" });
    }
  });
  ...
  app.get("/api/admin/stats", requireAdmin, async (req, res, next) => {
    try {
      ...
      const systemHealth = "good"; // Could be enhanced with actual health checks
  ...
  // Legacy alias for SimpleScopeFieldManager (returns same structure as old /scope-template)
  app.get("/api/services/:id/scope-template", requireAdmin, async (req, res) => {
    try {
      const serviceId = parseInt(req.params.id);
      const service = await storage.getService(serviceId);
      ...
      const scopeFields = await storage.getServiceScopeFields(serviceId);
      res.json({ template: { fields: scopeFields } });
    } catch (error) {
      console.error("Get scope template error:", error);
      res.status(500).json({ message: "Failed to fetch scope template" });
    }
  ...
  // Update SAF
  app.put("/api/service-authorization-forms/:id", requireManagerOrAbove, async (req, res, next) => {
    try {
      const id = parseInt(req.params.id);
  ...
  app.get("/api/dashboard/drilldown/:metric", requireAuth, async (req, res, next) => {
    try {
  ...
  app.post("/api/bulk-import/comprehensive-paste", requireManagerOrAbove, async (req, res, next) => {
    try {
      ...
      const columnMappingSchema = z.object({
        ...
        entityType: z.enum(['clients', 'contacts', 'contracts', 'licenses', 'hardware', 'saf', 'coc']),
        required: z.boolean(),
        dataType: z.string()
      });
      ...
      const results = {
        ...
        warnings: [] as string[],
            ...
            safForms: { created: 0, updated: 0 },
            cocForms: { created: 0, updated: 0 }
          }
      };
      ...
      await db.transaction(async (tx) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          let currentSafIdForRow: number | null = null;
          ...
          try {
                ...
                if (existingAsset.length > 0) {
                  ...
                  await tx.update(hardwareAssets)
                    .set({
                      ...
                      warrantyExpiry: hardwareData.warrantyExpiryDate ? new Date(hardwareData.warrantyExpiryDate) : existingAsset[0].warrantyExpiry,
                  ...
                  const [newAsset] = await tx.insert(hardwareAssets)
                    .values({
                      ...
                      warrantyExpiry: hardwareData.warrantyExpiryDate ? new Date(hardwareData.warrantyExpiryDate) : null,
            ...
            // 7. Process Service Authorization Forms (SAF)
            if (entityMappings.saf && contractId) {
              const safData = extractEntityData(row, entityMappings.saf, mappings);

              if (safData.safNumber && safData.title) {
                // Attempt to retrieve an existing service scope ID for this contract (optional)
                const scopeLookup = await tx.select({ id: serviceScopes.id })
                  .from(serviceScopes)
                  .where(eq(serviceScopes.contractId, contractId))
                  .limit(1);
                const relatedServiceScopeId: number | null = scopeLookup.length > 0 ? scopeLookup[0].id : null;

                // Determine status based on signed flag
                const status = safData.signed !== undefined ? (safData.signed ? 'approved' : 'pending') : 'pending';

                // Check existing SAF by safNumber
                const existingSaf = await tx.select()
                  .from(serviceAuthorizationForms)
                  .where(eq(serviceAuthorizationForms.safNumber, safData.safNumber))
                  .limit(1);

                let safId: number;
                if (existingSaf.length > 0) {
                  await tx.update(serviceAuthorizationForms)
                    .set({
                      clientId,
                      contractId,
                      serviceScopeId: relatedServiceScopeId,
                      title: safData.title,
                      requestedDate: safData.startDate ? new Date(safData.startDate) : existingSaf[0].requestedDate,
                      expiryDate: safData.endDate ? new Date(safData.endDate) : existingSaf[0].expiryDate,
                      status,
                      updatedAt: new Date()
                    })
                    .where(eq(serviceAuthorizationForms.id, existingSaf[0].id));

                  safId = existingSaf[0].id;
                  results.details.safForms.updated++;
                } else {
                  const [newSaf] = await tx.insert(serviceAuthorizationForms)
                    .values({
                      clientId,
                      contractId,
                      serviceScopeId: relatedServiceScopeId,
                      safNumber: safData.safNumber,
                      title: safData.title,
                      requestedDate: safData.startDate ? new Date(safData.startDate) : new Date(),
                      expiryDate: safData.endDate ? new Date(safData.endDate) : null,
                      status,
                      notes: safData.notes || null
                    })
                    .returning({ id: serviceAuthorizationForms.id });

                  safId = newSaf.id;
                  results.details.safForms.created++;
                }

                // Keep reference for potential COC linkage
                currentSafIdForRow = safId;
              } else {
                results.warnings.push(`Row ${i + 1}: Skipping SAF - missing safNumber or title`);
              }
            }

            // 8. Process Certificates of Compliance (COC)
            if (entityMappings.coc && contractId) {
              const cocData = extractEntityData(row, entityMappings.coc, mappings);

              if (cocData.cocNumber && cocData.title) {
                const status = cocData.signed !== undefined ? (cocData.signed ? 'active' : 'pending') : 'active';

                const existingCoc = await tx.select()
                  .from(certificatesOfCompliance)
                  .where(eq(certificatesOfCompliance.cocNumber, cocData.cocNumber))
                  .limit(1);

                if (existingCoc.length > 0) {
                  await tx.update(certificatesOfCompliance)
                    .set({
                      clientId,
                      contractId,
                      safId: currentSafIdForRow,
                      title: cocData.title,
                      issueDate: cocData.issueDate ? new Date(cocData.issueDate) : existingCoc[0].issueDate,
                      expiryDate: cocData.expiryDate ? new Date(cocData.expiryDate) : existingCoc[0].expiryDate,
                      status,
                      updatedAt: new Date()
                    })
                    .where(eq(certificatesOfCompliance.id, existingCoc[0].id));

                  results.details.cocForms.updated++;
                } else {
                  await tx.insert(certificatesOfCompliance).values({
                    clientId,
                    contractId,
                    safId: currentSafIdForRow,
                    cocNumber: cocData.cocNumber,
                    title: cocData.title,
                    issueDate: cocData.issueDate ? new Date(cocData.issueDate) : new Date(),
                    expiryDate: cocData.expiryDate ? new Date(cocData.expiryDate) : null,
                    status,
                    notes: cocData.notes || null
                  });

                  results.details.cocForms.created++;
                }
              } else {
                results.warnings.push(`Row ${i + 1}: Skipping COC - missing cocNumber or title`);
              }
            }
      ...
      results.message = results.success
        ? `Successfully imported ${results.recordsSuccessful} row(s).`
        : `Imported ${results.recordsSuccessful} row(s) with ${results.recordsFailed} failure(s).`;

      res.json(results);

    } catch (error) {
      console.error('Comprehensive bulk import error:', error);
      next(error);
    }
  }); // end POST /api/bulk-import/comprehensive-paste
